...
class Node():
    ...
    def distance(self, node):
        d = math.sqrt(math.pow((self.x - node.x), 2) + math.pow((self.y - node.y), 2))
        return d
...
class Graph():
    def __init__(self, nodes):
        self.nodes = dict()
        for n in nodes:
            node = Node(n, nodes[n][0], nodes[n][1])
            self.nodes[n] = node
        self.reachable_node = dict()
    ...
    def add_edges(self, edges):
        for i in range(len(self.nodes)):
            edge = edges[i]
            for e in edge:
                self.nodes[i].add_connected_node(self.nodes[e])

    def set_reachable(self):
        for i in self.visited_node:
            for j in self.nodes[i].children:
                if not self.nodes[i].children[j].visited:
                    if self.nodes[i].children[j].reachable:
                        f = self.nodes[i].f + self.nodes[i].distance(self.nodes[i].children[j])
                        if f < self.nodes[i].children[j].f:
                            self.nodes[i].children[j].f = f
                            self.nodes[i].children[j].last_node = self.nodes[i]
                    else:
                        self.nodes[i].children[j].reachable = True
                        self.reachable_node[self.nodes[i].children[j].value] = self.nodes[i].children[j]
                        f = self.nodes[i].f + self.nodes[i].distance(self.nodes[i].children[j])
                        self.nodes[i].children[j].f = f
                        self.nodes[i].children[j].last_node = self.nodes[i]
    ...
    def cal_h(self, goal):
        for i in self.nodes:
            self.nodes[i].h = self.nodes[goal].distance(self.nodes[i])

    def find_lowest_next_node(self):
        min_value = 10000
        min_index = -1
        for i in self.reachable_node:
            if not self.nodes[i].visited:
                if self.nodes[i].f + self.nodes[i].h < min_value:
                    min_value = self.nodes[i].f + self.nodes[i].h
                    min_index = i
        return min_index
...
def shortest_path(M, start, goal):
    ...
    if start == goal:
        return [start]
    graph = Graph(M.intersections)
    graph.add_edges(M.roads)
    graph.nodes[start].visited = True
    graph.nodes[start].reachable = True
    graph.visited_node[start] = graph.nodes[start]
    graph.cal_h(goal)
    while True:
        graph.set_reachable()
        next_index = graph.find_lowest_next_node()
        if next_index == goal:
            break
        else:
            graph.nodes[next_index].visited = True
            graph.visited_node[next_index] = graph.nodes[next_index]
    result_list = list()
    result_list.append(goal)
    current_node = graph.nodes[goal]
    while current_node.value != start:
        result_list.insert(0, current_node.last_node.value)
...
map_40 = Map(roads, intersections)
